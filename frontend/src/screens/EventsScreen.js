      ...
      const mapped = (Array.isArray(data) ? data : []).map((row) => {
        const subtitle =
          row.subtitle && row.subtitle.trim().length > 0
            ? row.subtitle
            : (row.description || "").slice(0, 140) + ((row.description || "").length > 140 ? "…" : "");
        return {
          // EventCard expects these:
          title: row.title,
          subtitle,
          date: "", // no date in your schema by design
        ...
        <PullToRefresh scrollerRef={scrollerRef} onRefresh={refresh}>
          <Container className="py-3 mb-5" tabIndex={-1}>
            <ButtonGroup className="mb-3 w-100">
              {filters.map((f, i) => (
                <ToggleButton key={i} type="radio" variant="outline-primary" name="filter" value={f}>
                  {f}
                </ToggleButton>
              ))}
            </ButtonGroup>
            ...
            <InfoBox title="Events">
              ...
              {!loading && !err && events.length === 0 && (
                <div className="text-muted">No events yet. Create one!</div>
              )}
              {!loading && !err && events.map((e, i) => <EventCard key={i} {...e} />)}
            </InfoBox>
          </Container>
        </PullToRefresh>
